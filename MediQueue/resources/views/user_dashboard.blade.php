                    ...
                    <div id="home" class="tab-pane fade in active">
                        ...
                        @else
                            ...
                            @foreach($meetings as $key => $meeting)
                                {{--<h4 style="text-align: center;margin-top: 40px;">{{$meeting->getAdmin($meeting->admin_id)->name}} on {{strftime('%Y-%m-%d %H:%M:%S',$meeting->attend_to)}}</h4>--}}
{{----}}
                                        ...
                                        <div class="panel-heading">
                                            <h4 class="panel-title">
                                                <a data-toggle="collapse" data-parent="#accordion" href="#collapse{{$key}}">
                                                    {{$meeting->getAdmin($meeting->admin_id)->name}} on {{strftime('%Y-%m-%d %H:%M:%S',$meeting->attend_to)}}</a>
                                            </h4>
                                        </div>
                                        <div id="collapse{{$key}}" class="panel-collapse collapse {{ $key == 0 ? 'in' : '' }}">
                                            <div class="panel-body"><strong>Your Message: </strong>{{$meeting->complaint}}</div>
                                        </div>
                                    </div>
                                </div>
                            @endforeach
                    ...
                    <div id="profile" class="tab-pane fade">
                        ...
                        <table class="table">
                            ...
                            <tr>
                                <td>Meetings</td>
                                <td>{!! \App\Complaint::where('added_by',Auth::user()->user_id)->get()->count() !!}</td>
                            </tr>
                        </table>
                    </div>
                    <div id="meetings" class="tab-pane fade">
                        ...
                        @else
                            ...
                            @foreach($omeetings as $key => $omeeting)
                                {{--<h4 style="text-align: center;margin-top: 40px;">{{$omeeting->getAdmin($omeeting->admin_id)->name}} on {{strftime('%Y-%m-%d %H:%M:%S',$omeeting->attend_to)}}</h4>--}}
                                <div class="panel-group" id="accordion">
                                    <div class="panel panel-default">
                                        <div class="panel-heading">
                                            <h4 class="panel-title">
                                                <a data-toggle="collapse" data-parent="#accordion" href="#collapse{{$key}}">
                                                    {{$omeeting->getAdmin($omeeting->admin_id)->name}} on {{strftime('%Y-%m-%d %H:%M:%S',$omeeting->attend_to)}}</a>
                                            </h4>
                                        </div>
                                        <div id="collapse{{$key}}" class="panel-collapse collapse {{ $key == 0 ? 'in' : '' }}">
                                            <div class="panel-body"><strong>Your Message: </strong>{{$omeeting->complaint}}</div>
                                        </div>
                                    </div>
                                </div>
                            @endforeach
...
//                    console.log()
                    $('.queue').html('' +
                            ...
                            '<span>Meeting details can be found below</span>' +
                            '<ul class="list-group">' +
                            '<li class="list-group-item"><span class="badge">'+d.time+'</span> Meeting Time</li>' +
                            '<li class="list-group-item"><span class="badge">'+d.admin+'</span> Admin</li>' +
                            '<li class="list-group-item"><span class="badge">'+d.complain+'</span> Your message</li>' +
                            '</ul>' +
